...
export default function PantryEquipmentForm() {
	const { id } = useParams();
	...
	useEffect(() => {
		api.getSubcategories(5).then(result => {
			setSubs(result);
	...
	return (
		<form className={styles.form} onSubmit={submit}>
			<div className={styles.formHeader}>
				<h3>{id ? 'Edit Pantry Equipment' : 'Add Pantry Equipment'}</h3>
				<p className={styles.categoryTag}>☕ Pantry Equipment</p>
			</div>
