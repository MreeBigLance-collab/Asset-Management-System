...
export default function MiscellaneousForm() {
	...
	useEffect(() => {
		api.getSubcategories(8).then(result => {
			setSubs(result);
	...
	return (
		<form className={styles.form} onSubmit={submit}>
			<div className={styles.formHeader}>
				<h3>{id ? 'Edit Miscellaneous Asset' : 'Add Miscellaneous Asset'}</h3>
				<p className={styles.categoryTag}>📦 Miscellaneous</p>
			</div>
			...
			<div className={styles.formGroup}>
				...
				<textarea
					placeholder="e.g., Warranty expires 15/02/2027, Special features, Usage notes"
					value={model.remarks}
			...
			<div className={styles.buttonGroup}>
				<button type="submit" className={styles.submitBtn}>{id ? 'Update Asset' : 'Add Asset'}</button>
